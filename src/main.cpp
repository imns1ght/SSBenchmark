...
int main(void) {
	...
	header << "During time (ms)" << setw(20) << "Sample Size";
	...
	for (auto i = 0; i < samplesN; i++) {
		...
		std::cout << "\033[1;35m>>>\033[0m SAMPLE [" << i+1 << "] [" 
			  << beginRange << ":" << sampleSize << ") \033[1;35m<<<\033[0m";
		...
		if (result == NULL) { // If the value is not found 	
			cout << endl << "\033[1;35m>>>\033[0m Value " << searchValue	     
		...
		numbers << fixed << std::chrono::duration <double, std::milli> (average).count() << setw(28)
